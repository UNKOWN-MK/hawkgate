...
int main(int argc, char **argv)
{
    ...
    {
        case START:    return parse_start(argc, argv, prog);
        case STOP:     return parse_stop(argc, argv, prog);
        case ADD:      return parse_add(argc, argv, prog);
        case DEL:      return parse_del(argc, argv, prog);
        case SHOW:     return parse_show(argc, argv, prog);
        case DETAILS:  return parse_details(argc, argv, prog);
        case PROTOCOL: return parse_proto(argc, argv, prog);
        default:
            print_help(prog);
            return FAILED;
    }
...
int start_action(const char *iface)
{
    ...
    if (ifindex <= 0)
        return FAILED;
    /* ── load BPF object ── */
    skel = hg_tc_bpf__open_and_load();
    if (!skel) 
    {
        fprintf(stderr, "hgctl: failed to open/load BPF object\n");
        return FAILED;
    }
    /* ── create clsact qdisc ── */
    hook.sz           = sizeof(hook);
    ...
    if (run_cmd_strict("tc qdisc replace dev ifb0 root fq","install FQ qdisc on ifb0 (upload EDT)") != SUCCESS)
        return FAILED;
...
int add_action(const char *iface, const char *ip,time_t expire, time_t idle,__u64 d_limit, __u64 u_limit, __u32 rate_id)
{
    ...
    {
        fprintf(stderr, "hgctl: add — map open failed ""(client=%d counter=%d rate=%d)\n",client_fd, counter_fd, rate_fd);
        if (client_fd  >= 0) close(client_fd);
        if (counter_fd >= 0) close(counter_fd);
    ...
    if (bpf_map_update_elem(client_fd, &ip_key, &client_val, BPF_ANY)) 
    {
        ...
        close(client_fd); close(counter_fd); close(rate_fd);
        return FAILED;
    }
...
int details_action(const char *iface)
{
    ...
    {
        perror("hgctl: calloc");
        close(counter_fd); close(client_fd);
        return FAILED;
    }
    ...
    while (ret == 0)
    {
        ...
        if (bpf_map_lookup_elem(counter_fd, &key, values) == 0)
        {
            ...
            {
                if (st.expiry_ns > 0 && st.expiry_ns <= (__u64)ts_boot.tv_sec * NSEC_PER_SEC)
            ...
            {
                case AUTH_OK: state_str = "AUTH_OK"; break;
                case EXPIRE:  state_str = "EXPIRE";  break;
                case BLOCK:   state_str = "BLOCK";   break;
                case IDLE:    state_str = "IDLE";    break;
                default:      state_str = "UNKNOWN"; break;
            }
            char policy_buf[32];
            snprintf(policy_buf, sizeof(policy_buf), "%u|%llukbps",policy_id, BPS_TO_KBIT(rate.rate_Bps));
            struct in_addr a = { .s_addr = key };
...
void show_protocols(void)
{
    ...
    if (l2_fd < 0 || l3l4_fd < 0) 
    {
        ...
        if (l2_fd   >= 0) close(l2_fd);
        if (l3l4_fd >= 0) close(l3l4_fd);
        return;
    }
